...
The `id` and `label` parameters to the **SelectParameterOption** constructor are required. Arbitrary keyword arguments such as "dim_col" and "order_by_col" can be specified to the **SelectParameterOption** constructor, which will be treated as custom fields to the parameter option. For more info, see the [Widget Parameters] page.
...
See the "Selection Test Sets" section of the [About squirrels.yml] page for more information on selection test sets.

For more details on the compile command, run `sqrl compile --help` or see the [sqrl compile] page. 
...
[About squirrels.yml]: ../concepts/squirrels-yml
